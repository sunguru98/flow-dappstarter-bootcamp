...
class Flow {
        ...
        args: [
          {
            ...
            value: publicKeys.map((o) => o.encodedPublicKey),
          },
          { value: tokens, type: t.UFix64 },
        ],
        gasLimit: 300,
      };
    } else {
  ...
  async deployContract(address, name, contract) {
    ...
    let options = {
      ...
      //roleInfo: { [Flow.Roles.ALL]: address},
      args: [
        { value: name, type: t.String },
        {
          value: Buffer.from(contract, 'utf8').toString('hex'),
          type: t.String,
        },
      ],
      gasLimit: 300,
    ...
    const contractEvent = result.events.find(
      (d) =>
        d.type === 'flow.AccountContractAdded' ||
        d.type === 'flow.AccountContractUpdated'
    );
  ...
  async getAccount(address) {
    let accountInfo = await fcl.send([fcl.getAccount(address)], {
      node: this.serviceUri,
    });
    ...
    codeLines.forEach((line) => {
      ...
      if (tokens[0] === 'import') {
        ...
        if (deployedContracts[contractRef]) {
          updatedCode +=
            prefix +
            line.replace(contractRef, deployedContracts[contractRef]) +
            NEWLINE;
        } else {
  ...
  async _processTransaction(tx, options) {
    ...
    if (options.roleInfo) {
      ...
      if (
        roles[Flow.Roles.AUTHORIZERS] &&
        Array.isArray(roles[Flow.Roles.AUTHORIZERS])
      ) {
        let authorizations = [];
