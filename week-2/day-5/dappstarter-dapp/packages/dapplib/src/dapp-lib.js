...
module.exports = class DappLib {
  ...
  // kittyItemsDestroyKittyItem
  // calls transactions/kittyitems/destroy_kitty_item.cdc
  //
  // signer/proposer/authorizer: data.signer
  static async kittyItemsDestroyKittyItem(data) {
    const config = DappLib.getConfig();
    let result = await Blockchain.post(
      {
        config,
  ...
  // kittyItemsIsCollectionPresent
  // calls scripts/kittyitems/is_collection_present.cdc
  //
  // signer/proposer/authorizer: none
  // Note: the return type should be DAPP_RESULT_BOOLEAN
  //
  static async kittyItemsIsCollectionPresent(data) {
    let result = await Blockchain.get(
      {
        config: DappLib.getConfig(),
        roles: {},
      },
      'kittyitems_is_collection_present',
      {
        address: { value: data.address, type: t.Address },
      }
    );
    ...
    return {
      ...
      label: 'Kitty Items Collection Is Present',
      result: result.callData,
    };
  }

  // kittyItemsReadCollectionIDs
