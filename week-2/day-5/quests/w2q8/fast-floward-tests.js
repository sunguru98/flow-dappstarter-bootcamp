...
describe('Flow Dapp Tests', async () => {
  ...
  describe('Fast Floward Tests', async () => {
    ...
    it(`safely fails when withdrawing kittyitem with wrong id`, async () => {
      ...
      try {
        ...
        assert.equal(
          res1.result,
          2,
          'Admin should have 2 KittyItems after fail'
        );
        assert.equal(
          res2.result,
          0,
          'User should have 0 KittyItems after fail'
        );
      }
    });
    ...
    it(`lists 2 kittyitems for sale`, async () => {
      ...
      let res2 = await DappLib.kittyItemsMarketReadSaleCollectionLength(
        testData4
      );
      ...
      assert.equal(
        res1.result[0],
        0,
        "User's Market collection should have a KittyItem with itemID 0"
      );
      assert.equal(
        res1.result[1],
        1,
        "User's Market collection should have a KittyItem with itemID 1"
      );
    ...
    it(`removes a kittyitem from sale`, async () => {
      ...
      const result1 = await DappLib.kittyItemsMarketReadSaleCollectionLength(
        testData2
      );
      // 6) call kittyItemsMarketReadSaleCollectionIDs with testData3 and store the result
      const result2 = await DappLib.kittyItemsMarketReadSaleCollectionIDs(
        testData3
      );
      // 7) assert the result from step 5) shows the SaleCollection has length 1
      assert.equal(result1.result, 1, 'User should have 1 kitty item for sale');
    ...
    it(`checks both admin and user have correct balances after purchase`, async () => {
      ...
      const result1 = await DappLib.kibbleGetBalance(testData1);
      // 4) call kibbleGetBalance with testData2 and store the result
      const result2 = await DappLib.kibbleGetBalance(testData2);
      // 5) assert the result from step 3) equals 5.0
      assert.equal(result1.result, 5.0, 'Admin has incorrect balance');
      // 6) assert the result from step 4) equals 25.0
      assert.equal(result2.result, 25.0, 'User has incorrect balance');
    });

    it(`checks for proper kitty items destroyed`, async function () {
      const testData1 = {
        signer: config.accounts[0],
      };
      ...
      await DappLib.kittyItemsDestroyKittyItem(testData1);
      const res1 = await DappLib.kittyItemsIsCollectionPresent(testData2);
      assert.equal(res1.result, false, 'User still has collection resource');
    });
  });
});
